#ifndef LP_MP_MULTIGRAPH_MATCHING_CONSTRUCTOR_HXX
#define LP_MP_MULTIGRAPH_MATCHING_CONSTRUCTOR_HXX
...
public:
    ...
    GRAPH_MATCHING_CONSTRUCTOR* add_graph_matching_problem(const std::size_t p, const std::size_t q, SOLVER& s)
    {
        ...
        auto* c = new GRAPH_MATCHING_CONSTRUCTOR(s);
        graph_matching_constructors.insert(std::make_pair(graph_matching({p,q}), c)); 
        return c;
    }
    ...
    //std::tuple<std::vector<std::size_t>, std::vector<std::size_t>, std::size_t, std::size_t,
    // matching factor p->q-unary, q->r-unary, p->r unary, r->q unary
    //> 
    auto
    get_triplet_consistency_factor_data(const triplet_consistency_factor& t) const
    {
        ...
        auto get_matching_data = [&](const std::size_t p, const std::size_t q, const std::size_t p_node) {
            if(p<q) {
                ...
                auto* f_pq = pq_constructor->left_mrf.GetUnaryFactor(p_node);
                ...
                auto* f_pq = pq_constructor->right_mrf.GetUnaryFactor(p_node);
        ...
        auto* f_pr_left = pr_constructor->left_mrf.GetUnaryFactor(p_node);
        ...
        auto* f_pr_right = pr_constructor->right_mrf.GetUnaryFactor(r_node);
            ...
            GRAPH_MATCHING_CONSTRUCTOR* pq_constructor, GRAPH_MATCHING_CONSTRUCTOR* qr_constructor, GRAPH_MATCHING_CONSTRUCTOR* pr_constructor) const
    {
        ...
        m_pq.send_message_to_right(*f_pq->GetFactor(), msg_val_pq, 1.0);
        m_pq.RepamRight(f, -1.0*msg_val_pq);
        m_pq.RepamLeft(*f_pq->GetFactor(), msg_val_pq);

        vector<REAL> msg_val_qr(qr_indices.size(), 0.0);
        m_qr.send_message_to_right(*f_qr->GetFactor(), msg_val_qr, 1.0);
        m_qr.RepamRight(f, -1.0*msg_val_qr);
        m_qr.RepamLeft(*f_qr->GetFactor(), msg_val_qr);

        array<REAL,1> msg_val_pr_left = {0.0};
        m_pr_left.send_message_to_right(*f_pr_left->GetFactor(), msg_val_pr_left, 1.0);
        m_pr_left.RepamRight(f, -msg_val_pr_left[0], 0);
        m_pr_left.RepamLeft(*f_pr_left->GetFactor(), msg_val_pr_left[0], 0);

        array<REAL,1> msg_val_pr_right = {0.0};
        m_pr_right.send_message_to_right(*f_pr_right->GetFactor(), msg_val_pr_right, 1.0);
        m_pr_right.RepamRight(f, -msg_val_pr_right[0], 0);
        m_pr_right.RepamLeft(*f_pr_right->GetFactor(), msg_val_pr_right[0], 0);

        const REAL after_lb = f.LowerBound() + f_pq->LowerBound() + f_qr->LowerBound() + f_pr_left->LowerBound() + f_pr_right->LowerBound();

        // revert changes
        m_pq.RepamLeft(*f_pq->GetFactor(), -1.0*msg_val_pq);
        m_qr.RepamLeft(*f_qr->GetFactor(), -1.0*msg_val_qr);
        m_pr_left.RepamLeft(*f_pr_left->GetFactor(), -msg_val_pr_left[0], 0);
        m_pr_right.RepamLeft(*f_pr_right->GetFactor(), -msg_val_pr_right[0], 0);
        ...
        for(std::size_t r=0; r<no_graphs; ++r) {
            for(std::size_t q=0; q<r; ++q) {
                for(std::size_t p=0; p<q; ++p) {
                    ...
                    auto* pr_constructor = get_graph_matching_constructor(p,r);
                    const auto p_no_nodes = pq_constructor->left_mrf.GetNumberOfVariables();
                    const auto q_no_nodes = pq_constructor->right_mrf.GetNumberOfVariables();
                    const auto r_no_nodes = pr_constructor->right_mrf.GetNumberOfVariables();
...
private:
    ...
    std::unordered_map<graph_matching, std::shared_ptr<GRAPH_MATCHING_CONSTRUCTOR>, graph_matching_hash> graph_matching_constructors; 
};


} // namespace LP_MP

#endif // LP_MP_MULTIGRAPH_MATCHING_CONSTRUCTOR_HXX
